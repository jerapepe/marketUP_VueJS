...
//const loggedIn = ref(store.state.isLoggedIn);
const usuario = ref({
  username: '',
  token: '',
});
...
const enviarDatos = () => {
  if (formData.value.name == "" && formData.value.email == "" && formData.value.username == "" && formData.value.password == "") {
    alertMessage.value = "Completar registro";
    showAlert.value = true;
  } else {
    fetch('http://192.168.0.73:8000/signup', {
      method: 'POST',
      headers: {
      ...
      .then(data => {
        alertMessage.value = data.isVerified;
        showAlert.value = true;
        //loggedIn.value = true;
      })
      .catch(error => {
        alertMessage.value = "Error al enviar datos";
        showAlert.value = true;
...
const enviar = () => {
  if (formData.value.username == "" && formData.value.password == "") {
    alertMessage.value = "Complete los datos";
    showAlert.value = true;
    usuario.value.username = "Invitado";
  } else {
    fetch('http://192.168.0.73:8000/signin', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      ...
      .then(data => {
        ...
        if (data.username != null) {
          usuario.value.username = data.username;
          usuario.value.token = data.token;
          localStorage.setItem('username', data.username);
          localStorage.setItem('token', data.token);
          store.commit('setLoggedIn', { value: true, username: data.username, token: data.token });
          router.push('/profile');
        }
      })
      .catch(error => {
        alertMessage.value = "Datos incorrectos: " + error;
        showAlert.value = true;
      });
  }
};
      ...
      <div class="form-container">
        ...
        <form class="form" @submit.prevent="enviar">
          <div class="input-group">
            ...
            <input type="text" name="username" id="username" v-model="formData.username" placeholder="">
          </div>
          <div class="input-group">
